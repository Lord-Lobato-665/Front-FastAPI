<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 p-6">
    <div class="mx-auto bg-white rounded-xl shadow-2xl overflow-hidden">
      <!-- Header -->
      <div
        class="bg-gradient-to-r from-indigo-600 to-indigo-800 p-6 text-white"
      >
        <h2 class="text-3xl font-bold">Análisis K-Means Clustering</h2>
        <p class="text-indigo-100 mt-2">
          Agrupa tus datos según patrones de comportamiento
        </p>
      </div>
      <div class="p-6 space-y-8">
        ...
        <div>
          ...
          >
            <div
              ...
              :key="col"
              @click="toggleColumn(col, !selectedColumns.includes(col))"
              :class="[
          ...
          <!-- Columnas seleccionadas (solo visualización) -->
          <div v-if="selectedColumns.length > 0" class="mb-6">
            ...
            <div class="flex flex-wrap gap-3">
              ...
              >
                ...
                <span
                  class="text-indigo-700 bg-white border border-indigo-300 px-3 py-1 rounded text-sm shadow-sm"
                >
                  {{ inputValues[col] }}
                </span>
              </div>
        ...
        <div>
          <label class="block text-lg font-medium text-gray-800 mb-2"
            >Cantidad de grupos (2 a 5):</label
          >
          <div class="flex items-center gap-4">
            ...
            />
            <span class="text-indigo-700 font-bold text-xl w-8 text-center">{{
              kValue
            }}</span>
          </div>
        </div>
        ...
        >
          <div
            class="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-6 border border-indigo-100 shadow-sm"
          >
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <!-- Grupo asignado -->
              <div
                class="bg-white p-4 rounded-lg border border-indigo-100 shadow-xs"
              >
                <h4
                  class="font-semibold text-gray-700 flex items-center gap-2 mb-2"
                >
                  <svg
            ...
            <div class="mt-6 space-y-4">
              ...
              >
                ...
                <p class="text-gray-600 mb-3">
                  {{ typedFixedDescriptions[idx] }}
                </p>
                <div
                  v-if="result.descripcion_cluster[idx]"
                  class="bg-blue-50 p-3 rounded border-l-4 border-blue-400"
                >
                  <p class="text-sm text-gray-700">
                    <span class="font-medium text-blue-700"
                      >Análisis del modelo:</span
                    >
                    {{ typedDescriptions[idx] }}
                  </p>
                </div>
              </div>
            </div>
...
const fixedDescriptions: Record<string, string> = {
  ...
  Avg_Daily_Usage_Hours:
    "El tiempo promedio diario en redes sociales puede indicar niveles de distracción, dependencia o evasión.",
  Sleep_Hours_Per_Night:
    "Las horas de sueño reflejan la calidad del descanso, lo cual tiene impacto directo en el bienestar general.",
  Mental_Health_Score:
    "El puntaje de salud mental brinda una visión sobre la estabilidad emocional y el estado psicológico actual.",
  Conflicts_Over_Social_Media:
    "Los conflictos asociados al uso de redes pueden reflejar relaciones tóxicas, malentendidos o estrés interpersonal.",
  Addicted_Score:
    "El puntaje de adicción estima el nivel de dependencia a redes sociales, afectando tanto rutinas como relaciones sociales.",
};
...
onMounted(async () => {
  ...
  if (userData) {
    const keys = Object.keys(userData).filter(k => availableColumns.value.includes(k))
    selectedColumns.value = keys.slice(0, 3)

    for (const key of selectedColumns.value) {
      const val = Number(userData[key])
      inputValues.value[key] = key === 'Age' ? Math.floor(val) : parseFloat(val.toFixed(2))
    }
  }
})
...
function toggleColumn(col: string, checked: boolean) {
  if (checked) {
    if (
      ...
      // Solo asigna valor si no existía ya
      if (inputValues.value[col] === undefined) {
        inputValues.value[col] = col === "Age" ? 18 : 5;
      }
    }
  } else {
...
onMounted(async () => {
  ...
  const userData = profileStore.userData;
  if (userData) {
    const keys = Object.keys(userData).filter(k =>
      availableColumns.value.includes(k)
    );

    // Solo precarga valores, no selecciona columnas
    for (const key of keys) {
      const val = Number(userData[key]);
      inputValues.value[key] =
        key === "Age" ? Math.floor(val) : parseFloat(val.toFixed(2));
    }
  }
});
...
const isValid = computed(() => {
  if (selectedColumns.value.length < 2 || selectedColumns.value.length > 3)
    return false;
...
watch(result, (newVal) => {
  if (newVal && newVal.user_info) {
    ...
    const fixedTexts = selectedColumns.value.map(
      (col) => fixedDescriptions[col] || "Sin análisis previo disponible."
    );
...
const predict = async () => {
  ...
  try {
    const payload: KMeansInput = {
      ...
      normalize: false,
      user_input: {
        values: selectedColumns.value.map((c) => inputValues.value[c]),
      },
    };
    ...
    error.value =
      e?.response?.data?.detail || e.message || "Error al procesar el análisis";
  } finally {
